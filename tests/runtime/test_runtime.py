...
def test_get_length_ex_onode():
    ...
    assert get_len_ex_factor(output_attrs_map["n2_1"]) == 1  # type: ignore
    assert get_len_ex_factor(output_attrs_map["n3_1"]) == 4  # type: ignore


class TestPAIRuntime:
    def test_get_len_ex_factor(self):
        onode_attrs = {
            "(0,1)": {LEN_EX_FACTOR_KEY: [0, 0, 0, 0, 0]},
            "(0,2)": {LEN_EX_FACTOR_KEY: [0, 0, 0, 0, 0]},
            "(1,2)": {LEN_EX_FACTOR_KEY: [0, 1, 1, 1, 1]},
            "(1,3)": {LEN_EX_FACTOR_KEY: [1, 1, 1, 2, 2]},
        }
        n_ex_onode = get_len_ex_factor(onode_attrs)  # type: ignore
        assert n_ex_onode == 3
    ...
    def test_encode(self):
        ...
        with fp.open("r") as f:
            inode_attrs_map = json.load(f)

        n_input_node = len(inode_attrs_map.keys())
        assert n_input_node == 2

        n_ts = 8
        cfg_map = PAIRuntime.gen_input_rtcfg_map(n_ts, inode_attrs_map)

        data = gen_random_array((cfg_map["inp1_1"].size,), dtype=np.uint8)
        _ = PAIRuntime.encode(
            data,
            cfg_map["inp1_1"].template,
            is_dest_online=cfg_map["inp1_1"].is_online,
        )

        assert len(cfg_map) == 2
        assert cfg_map["inp1_1"].size == 64 * n_ts
        assert cfg_map["inp2_1"].size == 32 * n_ts

        # timestep too large
        with pytest.raises(ValueError):
            _ = PAIRuntime.gen_input_rtcfg_map(
                OffCoreCfg.N_TIMESLOT_MAX + 1, inode_attrs_map
            )

    @skip_if_method_removed(PAIRuntime, "gen_input_frames_template")
    def test_gen_input_rtcfg_map_compare_old_method(self):
        # Validate the new implementation against the old one
        fp = TEST_DATA_CFG_DIR / "input_proj_info1.json"
        file_not_exist_fail(fp)
        ...
        n_ts = 8
        cfg_map = PAIRuntime.gen_input_rtcfg_map(n_ts, inode_attrs_map)
        input_frames_info = PAIRuntime.gen_input_frames_template(n_ts, inode_attrs_map)

        for i, name in enumerate(inode_attrs_map):
            assert np.array_equal(cfg_map[name].template, input_frames_info[i])

    @skip_if_method_removed(PAIRuntime, "gen_output_frames_template")
    def test_gen_output_rtcfg_map_compare_old_method(self):
        fp = TEST_DATA_CFG_DIR / "output_dest_info2.json"
        file_not_exist_fail(fp)

        with fp.open("r") as f:
            output_attrs_map = json.load(f)

        n_output_node = len(output_attrs_map.keys())
        assert n_output_node == 2

        n_ts = 1
        cfg_map = PAIRuntime.gen_output_rtcfg_map(n_ts, output_attrs_map)
        output_frames_info = PAIRuntime.gen_output_frames_template(
            n_ts, output_attrs_map
        )
        assert sum(part.size for part in cfg_map.values()) == 104

        for i, name in enumerate(output_attrs_map):
            assert np.array_equal(cfg_map[name].template, output_frames_info[i])

    def test_gen_output_rtcfg_map_ts_out_of_range(self):
        ...
        n_ts = 4
        cfg_map = PAIRuntime.gen_output_rtcfg_map(n_ts, output_attrs_map)
        assert sum(part.size for part in cfg_map.values()) == 800 * n_ts

        with pytest.raises(ValueError):
            _ = PAIRuntime.gen_output_rtcfg_map(
                OffCoreCfg.N_TIMESLOT_MAX + 1, output_attrs_map
            )

    def test_gen_output_rtcfg_map(self):
        fp = TEST_DATA_CFG_DIR / "output_dest_info2.json"
        file_not_exist_fail(fp)
        ...
        cfg_map = PAIRuntime.gen_output_rtcfg_map(1, output_attrs_map)
        assert sum(part.size for part in cfg_map.values()) == 104

    def test_decode1(self, fixed_rng):
        output_attrs_map = {
            "n2_1": {
                "(0,1)": {
                    "addr_axon": [0, 1, 2, 3, 4, 5, 6, 7],
        ...
        output_frames = np.array(
            [
                ...
                # Output to core (0,2) on chip (1,0)
        ...
        data = PAIRuntime.decode(output_frames, cfg_map)

        expected = np.zeros((len(output_attrs_map), n_ts, 8), dtype=PAYLOAD_DATA_DTYPE)
        # [onode][ts][axon]
        has_data_pos = {
            (0, 0, 3): 7,
            (0, 0, 5): 8,
        ...
        for i, name in enumerate(output_attrs_map):
            assert np.array_equal(data[name], expected[i])

    def test_decode2(self, fixed_rng):
        output_attrs_map = {
            "n2_1": {
                "(2,10)": {
                    "addr_axon": [0, 1, 2, 3, 4, 5, 6, 7],
    ...
    def test_decode3(self, fixed_rng):
        ...
        expected = np.zeros((len(output_attrs_map), n_ts, 10), dtype=PAYLOAD_DATA_DTYPE)
        # [onode][ts][axon]
        has_data_pos = {
            (0, 0, 0): 1,
            (0, 1, 0): 7,
    ...
    def test_decode_perf(self, n_axons, n_ts):
        ...
        print(f"Test case: n_axons={n_axons}, n_ts={n_ts}")
        print("Decoding with all checks")
        n = 1000
        t = timeit.timeit(
            lambda: PAIRuntime.decode(
                test_frames,
                cfg_map,
                raise_if_has_other_type=True,
                raise_if_duplicated=True,
                raise_if_not_matched=True,
            ),
            number=n,
        )
        print(f"time: {t/n:.5f}s")

        print("Decoding without all checks")
        t = timeit.timeit(
            lambda: PAIRuntime.decode(
                test_frames,
                cfg_map,
                raise_if_has_other_type=False,
    ...
    def test_decode4_onode_size_more1152(self, fixed_rng):
        ...
        output_frames = np.array(
            [
                ...
                # At ts=2
                0b1000_00001_00000_00000_00000_00000_00000_000_00000000001_00000010_00000100,
                0b1000_00001_00000_00000_00000_00000_00000_000_00000000010_00000010_00000010,
                0b1000_00001_00000_00000_00000_00000_00000_000_00000000011_00000010_00000011,
                0b1000_00001_00000_00000_00000_00000_00000_000_00000000000_00000011_00000001,
            ],
        ...
        expected = np.zeros(
            (len(output_attrs_map), n_ts, 1300), dtype=PAYLOAD_DATA_DTYPE
        )
        # [onode][ts][axon]
        has_data_pos = {
            (0, 0, 0): 1,
    ...
    def test_decode5_onode_size_more1152_multi_onodes(self, fixed_rng):
        ...
        expected = [expected_o1, expected_o2]
        for i, name in enumerate(output_attrs_map):
            assert np.array_equal(data[name], expected[i])

    def test_decode6_zero_output_frames(self):
        # Even if no output frames are given, decoding should be correct.
        fp = TEST_DATA_CFG_DIR / "output_dest_info_more1152.json"
        file_not_exist_fail(fp)
        ...
        data = PAIRuntime.decode(zero_oframes, cfg_map)
        assert all(d.all() == 0 for d in data.values())


@skip_if_version_less_than("paicorelib", REQUIRED_PLIB_VER_ONLINE_SUPPORT)
class TestRuntimeOnline:
    def test_gen_input_rtcfg_map(self):
        ...
        assert len(cfg_map) == 1
        assert cfg_map["InputProj_0"].size == 1200 * n_ts
    ...
    def test_gen_output_rtcfg_map_ts_out_of_range(self):
        ...
        n_output_node = len(output_attrs_map)
        assert n_output_node == 1

        n_ts = 7
        cfg_map = PAIRuntime.gen_output_rtcfg_map(n_ts, output_attrs_map)
        assert sum(part.size for part in cfg_map.values()) == 200 * n_ts

        with pytest.raises(ValueError):
            _ = PAIRuntime.gen_output_rtcfg_map(
                OnCoreCfg.N_TIMESLOT_MAX + 1, output_attrs_map
            )
    ...
    def test_decode1(self, fixed_rng):
        ...
        expected = np.zeros((len(output_attrs_map), n_ts, 16), dtype=PAYLOAD_DATA_DTYPE)
        # [onode][ts][axon]
        spike_pos = [
            (0, 0, 4),
            (0, 0, 6),
    ...
    def test_decode2(self, fixed_rng):
        ...
        fixed_rng.shuffle(output_frames)
        # Disable `raise_if_not_matched`
        data = PAIRuntime.decode(output_frames, cfg_map, raise_if_not_matched=False)

        expected = np.zeros(
            (len(output_attrs_map), n_ts, 200), dtype=PAYLOAD_DATA_DTYPE
        )
        has_spike_pos = [
            (0, 0, 0),
            (0, 0, 2),
    ...
    def test_decode3_onode_size_more1152(self, fixed_rng):
        ...
        expected = np.zeros(
            (len(output_attrs_map), n_ts, 1200), dtype=PAYLOAD_DATA_DTYPE
        )
        # [onode][ts][axon]
        has_spike_pos = [
            (0, 0, 0),
...
def shuffle_otf3(
    otf3: list[FrameArrayType], rng: np.random.Generator
) -> FrameArrayType:
...
class TestReadNeuronVoltage:
    ...
    def test_gen_read_attr_frames(self, fp):
        ...
        for neu_phy_loc in neu_phy_locs.values():
            ...
            tframe3 = PAIRuntime.gen_read_neuron_attrs_frames(
            ...
            tframe3_2 = PAIRuntime.gen_read_neuron_attrs_frames(
    ...
    def test_decode_voltage_onebyone1(self, monkeypatch, fixed_rng):
        ...
        expected_v = np.random.randint(-500, 500, size=(n_neuron,), dtype=VOLTAGE_DTYPE)
        supposed_addr = [
        ...
        for loc in neu_phy_locs.values():
            decoded_v = PAIRuntime.decode_voltage(loc, shuffled, "onebyone")
    ...
    def test_decode_voltage_contiguous(self, test_model_dir: Path):
        ...
        for loc in neu_phy_locs.values():
            n_neuron = get_n_neuron_from_phy_loc(loc)
            decoded_v = PAIRuntime.decode_voltage(loc, otframes, "contiguous")

            if test_model_dir.name in ["model3", "model4"]:
                expected_v = np.arange(1, n_neuron + 1, dtype=VOLTAGE_DTYPE)
            else:
                expected_v = np.arange(1, n_neuron + 1, dtype=np.int8).astype(
                    VOLTAGE_DTYPE
                )
